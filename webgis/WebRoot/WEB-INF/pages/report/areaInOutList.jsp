...
<body>
		<div id="toolbar">		
			...
			<form id="queryForm" action="<%=ApplicationPath%>/report/inAreaList.action">
			   <input type="hidden" name="childType" value="InArea" />	
			   <input type="hidden" name="queryId" value="selectAlarms" />		    	    
			   <input type="hidden" name="fileName" value="进出围栏记录" />	  
	      ...
	      <a id="btnQuery" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-search'" >查询</a>&nbsp;
		   ...
		   <a id="btnExport" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-excel'" onclick="Utility.excelExport('<%=ApplicationPath%>/data/excelExport.action');">导出</a><!--调用utility.js-->
				...
				<table id="queryGrid" class="easyui-datagrid" title="" style="width:100%;"
						...
						url:'<%=ApplicationPath%>/report/inAreaList.action',method:'post'">
					<thead>
						<tr>
								...
								<th data-options="field:'location'"  width="15%">围栏名称</th>
								<th data-options="field:'startTime'"  width="15%">进入时间</th>
								<th data-options="field:'endTime',formatter:getEndTimeColumn"  width="15%">离开时间</th>
								<th data-options="field:'timeSpan'"  width="12%">停留时长</th>
