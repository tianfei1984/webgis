...
<style>
 ...
 <script>
        $(window).ready(function () {
			...
			//ajax填充下拉框数据
			 $("#configType").lookup({category:"TerminalMenu", selectedValue:2});
		...
		function doSubmit()
		{
			...
			//{
						...
						{
				            $(".commandMsg").html("请录入菜单项");
						}

	                    var url = "<%=ApplicationPath%>/command/terminalMenu.action";
...
<script>
 ...
 <BODY>
 <form id="EntityForm" method="post" action="<%=ApplicationPath%>/command/terminalMenu.action">
        <input type="hidden"  name="menuList"  id="menuList" />
        <input type="hidden"  name="vehicleId"  id="vehicleId" value="${vehicleId}"/>
		信息菜单设置:&nbsp;&nbsp;<select id="configType" name="configType"  style="width: 150px;"></select>&nbsp;&nbsp;
