...
public class VehicleAction extends PersistenceAction {
	...
	public String save() {
		...
		if (vehicleId > 0) {
			...
			if (!oldVd.getPlateNo().equals(vd.getPlateNo()) || oldVd.getPlateColor() != vd.getPlateColor()) {
				VehicleInfoModifyRecord vr = new VehicleInfoModifyRecord();
			...
			if (!oldVd.getSimNo().equals(vd.getSimNo())) {
				VehicleInfoModifyRecord vr = new VehicleInfoModifyRecord();
				vr.setVehicleId(vehicleId);
		...
		try {
			...
			if (vd.getTermId() > 0) {
				try {
					t = (Terminal) baseService.load(Terminal.class,vd.getTermId());
				} catch (Exception e) {
					log.error(e.getMessage(), e);
				}
			} else {
