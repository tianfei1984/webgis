...
public class RouteAction extends PersistenceAction {
	...
	public String viewArea() {
		...
		if (this.isAuthorized(permissionName)) {
			this.allowEdit = "yes";
			// 超级用户可以编辑和删除所有标记
			if (enclosure.getEntityId() > 0
					&& u.getUserFlag() != UserInfo.USER_FLAG_SUPER_ADMIN) {
				String strUserId = "" + u.getEntityId();
				this.allowEdit = strUserId.equals(enclosure.getOwner()) ? "yes"
						: "no";
			}
		}
	...
	public String viewMarker() {
		...
		// 是否有编辑权限
		this.allowEdit = "no";
		if (this.isAuthorized(FunctionConstant.FUNC_MARKER)) {
			this.allowEdit = "yes";
			// 超级用户可以编辑和删除所有标记
			if (enclosure.getEntityId() > 0
					&& u.getUserFlag() != UserInfo.USER_FLAG_SUPER_ADMIN) {
	...
	public String saveArea() {
		try {
			...
			if (enclosure.getEntityId() == 0) {
				...
				UserInfo u = this.getOnlineUser();
				enclosure.setOwner("" + u.getEntityId());
			}
